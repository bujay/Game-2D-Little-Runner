...
func _ready():
	windowModeButton.connect("pressed", self, "on_window_mod_pressed")
	backButton.connect("pressed", self, "on_back_pressed")
	musicRangeControl.connect("percentage_changed", self, "on_music_volume_changed")
	sfxRangeControl.connect("percentage_changed", self, "on_sfx_volue_changed")
	update_display()
...
func on_sfx_volue_changed(percent):
